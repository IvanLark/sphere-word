...
export default function Header({ className, leadingBtn, middleElement, trailingBtn }: HeaderProps) {
	...
	return (<>
		<div className={`w-full h-16 fixed rounded-md border-2 border-black bg-white flex items-center overflow-hidden ${className}`}>
			{leadingBtn ? leadingBtn : <button title="Back" className="btn-trans size-16 rounded-md border-r-2 border-black group"
				onClick={() => { navigate(-1); }}>
				<div className="btn-scale-xl"><ArrowBack style={{ fontSize: "2.5rem" }} /></div>
			</button>
			}
			{/* 中间 */}
			{middleElement instanceof Element ? middleElement : <div className="flex-1 text-center text-3xl font-bold">{middleElement}</div>}
			{trailingBtn ? trailingBtn : <button title="Menu" className="btn-trans size-16 rounded-md border-l-2 border-black group">
