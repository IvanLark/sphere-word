...
export default function Review() {
	...
	const [remainingWords, setRemainingWords] = useState(['make', 'break', 'run', 'jump', 'walk', 'talk', 'listen', 'write', 'draw', 'paint', 'play', 'drink', 'eat']);
	...
	return (
		<>
			...
			} middleElement={`还剩${remainingWords.length}个单词`}
			/>
			<div className="w-full h-[calc(100vh-4rem)] flex flex-col">
				<div className="flex flex-1 flex-col gap-4 items-center justify-center">
					<h2 className="text-7xl font-bold">{word}</h2>
			...
			<RemainingWordsWin remainingWordsWinOpen={remainingWordsWinOpen} remainingWords={remainingWords} handleWordClick={handleWordClick} onClose={() => { setRemainingWordsWinOpen(false) }} />
		</>
	)
}
