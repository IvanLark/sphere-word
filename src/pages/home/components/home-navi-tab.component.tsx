...
export default function HomeNaviTabComponent({ title, icon, path, disabled }: HomeNaviTabProps) {
	const navigate = useNavigate();
	return (
		<button className="btn-scale btn-white w-80 p-4 rounded-xl border-2 border-black relative text-5xl font-bold"
						onClick={() => { if (disabled) { toastUtil.error('暂不支持该功能！'); return; } navigate(path); }}>
