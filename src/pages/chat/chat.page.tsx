...
import ChatArea from "./components/chat-area.component.tsx";
import { useLocation, useNavigate } from "react-router-dom";
import { ChatMessage } from "./chat.types.ts";
import { BASE_URL } from "../../api/api.constants.ts";
        ...
        trailingBtn={
          <button title="Menu" className="btn-trans size-16 rounded-md border-l-2 border-black group">
            <div className="btn-scale-xl"
                 onClick={() => { if (chatData.chatState !== 'generating') { navigate('/'); } else { toastUtil.error('生成完成前不能跳转'); } }}>
              <HomeOutlined style={{ fontSize: "2.5rem" }} />
            </div>
          </button>
        }
