...
import Input from "../../common/components/input.component.tsx";
import {toastUtil} from "../../common/utils/toast.util.tsx";
import {useNavigate} from "react-router-dom";
import {UserAuthData} from "../../api/auth/auth.types.ts";
import {login, signup} from "../../api/auth/auth.methods.ts";
...
export default function Auth() {
	...
	function handleLogin () {
		login(userData).then(response => {
			...
			toastUtil.error(`登录失败: ${error.response.data.msg}`);
		}).finally(() => {
	...
	function handleSignUp () {
		signup(userData).then(response => {
			...
			.catch((error: Error) => {
				toastUtil.error(`注册失败: ${error.message}`);
			}).finally(() => {
			setUserData(initUserData);
		});
	}
