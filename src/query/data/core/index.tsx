...
import Accordion_word_core from "../../../components/Accordion.tsx";
  ...
  return (
    <div className="w-full rounded-b-xl bg-white p-4">
      <div className="flex flex-col gap-5">
        ...
        {/* // ** AI解析 */}
        <Accordion_word_core title="AI解析" child={<Markdown>{wordCoreData.data.ai.DictionaryByGPT4}</Markdown>} titleClass="bg-blue-400 text-yellow-300 font-bold" childClass="bg-blue-400 text-white" />
        ...
        {["例句", "助记", "单词新解", "同义词", "形近词", "搭配", "替换", "派生词", "词根", "词源"].map((title, index) =>
          <Accordion_word_core key={index} title={title} child={<p className="" dangerouslySetInnerHTML={{ __html: wordCoreData.data.ai.Eudic[title as AiEudicTypes] }}></p>} titleClass="bg-blue-400 text-yellow-300 font-bold" childClass="bg-blue-400 text-white" />
